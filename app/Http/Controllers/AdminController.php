...
class AdminController extends Controller
{
    ...
    public function exportCsv(): StreamedResponse
    {
        ...
        $callback = function () {
            ...
            // CSVヘッダー
            fputcsv($handle, [
                'No.', '名前', 'メール', '登録日付', '携帯電話番号',
                '生年月日', '性別', '居住地（郵便番号）', '居住地（都道府県）',
                '居住地（市区町村）', 'メールマガジンの配信'
            ]);

            $users = User::distinct()->select(
                'users.id',
                'users.name',
                'users.email',
                'users.created_at',
                ...
                'addresses.ken_name',
                'addresses.city_name',
                'user_profiles.email_receive'
            )
                ->leftJoin('user_profiles', 'user_profiles.user_id', '=', 'users.id')
                ->leftJoin('addresses', 'addresses.city_id', '=', 'user_profiles.city_id')
            ...
            foreach ($users as $index => $user) {
                fputcsv($handle, [
                    ...
                    "'" . ($user->phone ?? ''),
                    substr($user->birthday, -1) == '-' ? $string = rtrim($user->birthday, "-") : $user->birthday,
                    $user->sex === 1 ? '男' : '女',
                    $user->zip ?? '',
                    $user->ken_name ?? '',
                    $user->city_name ?? '',
                    $user->email_receive ? 'Yes' : 'No',
                ]);
            }
            fclose($handle);
        };
    ...
    public function user_profile_update(Request $request)
    {
        ...
        if(isset($request->fullname)){
            ...
            $profile->fullname_kana = $request->fullname_kana;
        }else{
    ...
    {
        $jober_id = $id;
        $jobs = Job::where('jober_id', $jober_id)->whereIn('post_status', [0, 1, 2])->orderBy('content_updated_at', 'DESC')->get();
