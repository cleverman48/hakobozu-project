...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $new_jobs = Job::where('post_status', 1)->orderBy('updated_at', 'DESC')->take(4)->get();
    ...
    public function area_search($area_id)
    {
        ...
        $jobs = Job::select($jobSelector)
            ...
            ->appends(request()->input());
        $working_place = $this->get_working_place();
        $search_area = Address::select(DB::raw("CONCAT(ken_name, ' ', city_name) as area_name"))->where('city_id', $area_id)->groupBy('city_id', 'ken_name', 'city_name')->first();
        foreach ($jobs as $job) {
            $job->displayDaysAgo = $this->getDisplayDaysAgo($job->content_updated_at);
            $working_places = [];
            foreach ($working_place as $key => $place) {
        ...
        return view('front.search_result')->with(compact('jobs', 'prefecture', 'categories', 'search_area'));
    }
    ...
    public function category_search($category)
    {
        ...
        $jobs = Job::where('post_category', 'like', '%' . $category . '%')
            ->where('post_status', 1)->orderBy('jobs.content_updated_at', 'DESC')->paginate(20)
            ->appends(request()->input());
        $working_place = $this->get_working_place();
        foreach ($jobs as $job) {
            $job->displayDaysAgo = $this->getDisplayDaysAgo($job->content_updated_at);
    ...
    public function bid_post_unauth(Request $request)
    {
        $message = [
            ...
            'ken_id.required' => '都道府県は必須です。',
            'city_id.required' => '市区町村は必須です。',
            'full_name.required' => '氏名は必須です。',
            'email.required' => 'メールアドレスは必須です。',
            'password.required' => 'パスワードは必須です。',
            'sei_mei.required' => '性別は必須です。',
            'phone.required' => '携帯番号は必須です。',
            'agree.required' => '利用規約は必須です。',
            'password.min' => 'パスワードは8桁以上でなければなりません。',
            'password.confirmed' => 'パスワードが一致しません。',
            'email.email' =>  '正確なメールフォームを入力してください！'
        ];
    ...
    public function user_profile($user_id, $bid_id = null)
    {
        $interviews = null;
        $bid_content = null;
        $user = User::where('id', $user_id)->first();
        ...
        if ($bid_id) {
            $interviews = \App\Models\InterviewTime::where('bid_id', $bid_id)->get();
            $bid_content = \App\Models\Bid::where('id', $bid_id)->value('bid_content');
        }
        //        dd($address);
        return view('front.user_profile')->with(compact('user', 'profile', 'address', 'interviews', 'bid_content'));
    }
    ...
    private function getDisplayDaysAgo($contentUpdatedAt): string
    {
        $updatedAt = Carbon::parse($contentUpdatedAt);
        $now = Carbon::now();
        $hoursDiff = $updatedAt->diffInHours($now);
        $daysDiff = $updatedAt->diffInDays($now);
        $monthsDiff = $updatedAt->diffInMonths($now);

        if ($monthsDiff >= 1) {
            $displayDaysAgo = $updatedAt->format('Y.m.d');
        } elseif ($daysDiff >= 1) {
            $displayDaysAgo = '約' . $daysDiff . '日前に更新';
        } else {
            $displayDaysAgo = '約' . $hoursDiff . '時間前に更新';
        }

        return $displayDaysAgo;
    }
}
